"""NeedleSolution —— 修复策略基类（零依赖）。

用户扩展契约：继承 ``NeedleSolution`` 并实现 ``can_fix`` / ``fix`` 两个方法，
再通过 :func:`needle.register_solution` 注册即可被自动发现并纳入责任链。
"""
...
class NeedleSolution(ABC):
    ...
    def __init__(self, context: dict):
        ...
        self._next_solution: Optional["NeedleSolution"] = None
    ...
    def set_next(self, solution: "NeedleSolution") -> "NeedleSolution":
